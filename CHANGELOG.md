...
- Endpoint API `/bypass` (HTTP POST) untuk memungkinkan pengiriman data tugas secara langsung tanpa melalui antarmuka web.
...
- Memperbaiki urutan daftar aktivitas (task) pada detail logbook agar diurutkan berdasarkan waktu secara _ascending_ (dari awal hingga terbaru).
...
- Menggunakan `old()` _helpers_ pada semua formulir agar isian pengguna tidak hilang saat validasi gagal.
